...
description: >
   网易云音乐智能助手，以模型判断为核心，通过 ncm-cli (npm v0.1.6, 登录后全功能) 执行所有操作。
...
这是一个以**模型判断为核心**的音乐助手。底层由 `ncm-cli` (npm v0.1.6) 统一提供所有功能：

| 能力 | 命令（登录后） |
|---|---|
| **搜索** | `search song/album/playlist/all` |
| **推荐** | `recommend daily/heartbeat/fm` |
| **播放控制** | `play/pause/next/prev/volume/state/queue` |
| **红心** | `song like/dislike` |
| **歌单** | `playlist collected/radar/create/get/tracks/add/remove` |
| **用户数据** | `user favorite/history/listen-ranking` |
| **歌词** | `song lyric` |

**⚠️ 登录门控**：ncm-cli v0.1.6 的命令是登录门控的。登录前只显示约 10 个播放控制命令，登录后才显示全部。先用 `ncm-cli login --check` 确认登录态。

**ncm-go 已废弃并删除** —— 该 Go 二进制依赖 Playwright Chromium，用户不接受。所有操作通过 ncm-cli 完成。

核心能力链：
```
理解需求 → 分析偏好 → 制定搜索策略 → ncm-cli search → 筛选结果 → 语义化推荐 → ncm-cli 播放 + 可选建单
```
...
所有操作通过 `ncm-cli` 完成（登录后），**找不到搜索/推荐/红心等命令时先登录**（`ncm-cli login --check`）。

- **搜索歌曲**：`ncm-cli search song --keyword xxx --output json`
- **搜索歌单**：`ncm-cli search playlist --keyword xxx --output json`
- **播放歌曲**：`ncm-cli play --song --encrypted-id <32位hex> --original-id <数字ID>`
- **播放控制**：`ncm-cli pause / resume / next / prev`
- **每日推荐**：`ncm-cli recommend daily --output json`
- **心动模式**：`ncm-cli recommend heartbeat --playlistId <enc_id> --count 20 --output json`
- **红心歌曲**：`ncm-cli song like --id <32位hex>`
- **歌单列表**：`ncm-cli playlist collected --output json`
- **用户红心歌单**：`ncm-cli user favorite --output json`
...
若用户同意或主动要求，请执行以下操作（使用 ncm-cli，需要已登录）：

```
# 创建歌单 → 获取歌单加密ID
ncm-cli playlist create --name "歌单名" --output json
# 从返回的 id 字段取加密ID
# 添加歌曲到歌单
ncm-cli playlist add --id <歌单加密ID> --songIds <歌曲加密ID1,歌曲加密ID2,...>
# 验证 trackCount 确认写入成功
ncm-cli playlist get --id <歌单加密ID> --output json
# 在推荐输出中附上歌单链接（用明文 originalId）
```
