...
- **Self-verify before reporting**: hard rule — never hand the user something unverified. After config changes, run the tool yourself (`waybar -l debug`, `swaymsg`, `playerctl -l`) and inspect the actual output (GTK widget tree, IPC responses, helper stdout) before reporting. User explicitly does not want to be the test driver. Diagnostic moves: `process(action="log")` to read background-process stderr; `waybar -l debug > /tmp/waybar.log 2>&1` to capture per-output widths; `cat /proc/<pid>/environ | tr '\0' '\n' | grep SWAYSOCK` to verify env; `swaymsg -t get_outputs` to confirm IPC works.
